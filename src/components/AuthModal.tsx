...
import { loginUser, registerUser, setTokens, getMe } from "../lib/api";
...
export function AuthModal({ mode, onClose, onSuccess }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess({
        name: me.name ?? name ?? email.split("@")[0],
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white">
        <CardHeader className="relative">
          ...
          <CardTitle>{currentMode === "login" ? "Welcome Back" : "Create Account"}</CardTitle>
          <CardDescription>
            {currentMode === "login"
