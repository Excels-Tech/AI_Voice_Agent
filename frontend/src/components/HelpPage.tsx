  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400">Get help with VoiceAI and find answers to your questions</p>
      </div>
        ...
        <CardContent className="p-6">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-12 h-12 bg-slate-800 border-slate-700 text-white placeholder:text-slate-500"
            />
          </div>
        </CardContent>
      </Card>
      ...
      {/* Quick Resources */}
      <div>
        <h2 className="text-white mb-4">Quick Resources</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {resources.map((resource) => (
            <Card
              key={resource.title}
              className="bg-slate-900 border-slate-800 hover:border-slate-700 hover:shadow-lg transition-all cursor-pointer"
              onClick={resource.action}
            >
              <CardContent className="p-6">
                <div className={`inline-flex p-3 rounded-lg bg-gradient-to-br ${resource.color} mb-4 shadow-lg`}>
      ...
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader className="border-b border-slate-800">
          ...
          <CardDescription className="text-slate-400">Our team is here to help you</CardDescription>
        </CardHeader>
        <CardContent className="p-6">
          <div className="grid md:grid-cols-3 gap-6">
            <button
              onClick={() => toast.success("Opening live chat...")}
              className="p-6 border-2 border-slate-800 rounded-lg hover:border-blue-500 hover:bg-blue-500/10 transition-all text-center"
            >
              <div className="size-12 rounded-full bg-blue-500/10 border border-blue-500/30 flex items-center justify-center mx-auto mb-3">
                <MessageCircle className="size-6 text-blue-400" />
              </div>
              <h3 className="text-white mb-1">Live Chat</h3>
              <p className="text-slate-400 text-sm mb-2">Average response: 2 minutes</p>
              <Badge className="bg-green-500 shadow-lg shadow-green-500/30">Available Now</Badge>
            </button>

            <button
              onClick={() => toast.success("Opening email form...")}
              className="p-6 border-2 border-slate-800 rounded-lg hover:border-purple-500 hover:bg-purple-500/10 transition-all text-center"
            >
              <div className="size-12 rounded-full bg-purple-500/10 border border-purple-500/30 flex items-center justify-center mx-auto mb-3">
              ...
              <p className="text-purple-400 text-sm">support@voiceai.app</p>
            </button>

            <button
              onClick={() => toast.success("Scheduling call...")}
              className="p-6 border-2 border-slate-800 rounded-lg hover:border-green-500 hover:bg-green-500/10 transition-all text-center"
            >
              <div className="size-12 rounded-full bg-green-500/10 border border-green-500/30 flex items-center justify-center mx-auto mb-3">
                <Phone className="size-6 text-green-400" />
              </div>
              <h3 className="text-white mb-1">Schedule Call</h3>
              <p className="text-slate-400 text-sm mb-2">Talk to our team</p>
              <p className="text-green-400 text-sm">Book a time slot</p>
            </button>
          </div>
        </CardContent>
      </Card>
      ...
      <div>
        ...
        <div className="space-y-4">
          {(searchQuery ? filteredFaqs : faqs).map((category) => (
            <Card key={category.category} className="bg-slate-900 border-slate-800">
              <CardHeader className="border-b border-slate-800">
                ...
                <div className="space-y-4">
                  {category.questions.map((faq, idx) => (
                    <details key={idx} className="group">
                      <summary className="flex items-center justify-between cursor-pointer p-4 bg-slate-800/50 hover:bg-slate-800 rounded-lg transition-colors">
                      ...
                      <div className="p-4 text-slate-300 border-l-2 border-blue-500 ml-4 mt-2">
                        {faq.a}
                      </div>
                    </details>
                  ))}
          ...
          {searchQuery && filteredFaqs.length === 0 && (
            <Card className="bg-slate-900 border-slate-800">
              <CardContent className="p-12 text-center">
                <HelpCircle className="size-16 mx-auto mb-4 text-slate-700" />
      ...
      <Card className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border-green-500/30">
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" onClick={() => toast.success("Opening status page...")} className="border-slate-700 text-slate-300 hover:bg-slate-800 hover:text-white">
              View Status Page
            </Button>
          </div>
        </CardContent>
