...
This is a code bundle for the AI Meeting Voice Agent Workflow. The original project is available at https://www.figma.com/design/AMZK7gUHet9mB2R2zsaRLR/AI-Meeting-Voice-Agent-Workflow.

## Frontend (new)
- Location: `frontend/` (Vite + React + TypeScript).
- Install deps: `cd frontend && npm install`.
- Run dev server: `npm run dev` (default port 3000 from `vite.config.ts`).
- Build for backend hosting: `npm run build` (outputs to `frontend/build`). Copy the build into `backend/static/frontend/` or run `./build.sh` from the repo root to build dependencies and copy automatically.

## Backend API (FastAPI)

A production FastAPI backend lives in `backend/` and mirrors the data that the UI displays (agents, call logs, analytics, billing, etc.).
...
4. Render commands (also in `render.yaml`):
   - Build: `pip install --no-cache-dir -r requirements.txt` (see `buildCommand` in the YAML for the full frontend build + copy step)
